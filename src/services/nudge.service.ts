...
import Nudge, { INudge } from '../models/nudge.model.js';
...
export const sendNudge = async (
  ...
  if (!toUserId) {
    throw new ExpenseError('INVALID_NUDGE', 'toUserId is required', 422);
  }
  ...
  await notificationService.createNotification(
    ...
    'expense_nudge',
    'Payment reminder',
    `${from?.fName ?? 'Someone'} is reminding you to settle ${
      balances.currency ?? ''
    }${debt.amount}`,
    {
      groupId,
